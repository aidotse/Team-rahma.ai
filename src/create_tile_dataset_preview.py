...
def main(unused_argv):
    ...
    for (input_files, target_files, size_text) in tqdm(zip(
        ...
        # contains name part "AssayPlate_Greiner_#655090_D04"
        # and channel part "T0001F006L01A04Z01C04"

        df_rows = []
        unique_slide_names = np.unique(np.array(['_'.join(os.path.basename(fn).split('_')[:-1]) for fn in input_files]))
